...
var runChecks = function (ast, fullPath, lines, config) {
    ...
    linters = mergeLinters(defaultLinters, config);
...
var mergeLinters = function (defaultLinters, config) {
    if (!config.linters) {
        return defaultLinters;
    }

    if (!(config.linters instanceof Array)) {
        throw new Error('linters should be an array of file paths');
    }

    return defaultLinters.concat(config.linters.map(linterPath => require(linterPath)));
};
