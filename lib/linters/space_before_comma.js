...
module.exports = {
    name: 'spaceBeforeComma',
    nodeTypes: ['arguments', 'parentheses'],
    message: 'Commas should%s be preceded by %s space.',

    lint: function spaceBeforeCommaLinter (config, node) {
        var results = [];
        ...
        node.forEach('operator', function (element, index) {
            ...
            switch (config.style) {
                ...
                default:
                    throw new Error(
                        'Invalid setting value for spaceBeforeComma: ' + config.style
                    );
            }
        });
